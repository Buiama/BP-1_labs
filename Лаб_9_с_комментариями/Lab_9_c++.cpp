...
string makeSentence(string str) { // зводить до вигляду рядка
...
int wordsCounter(string str) { // підраховує слова в реченні (можна було зробити в makeSentence, але повертатиме два значення)
    int w = 0;
    for (int i = 0; i < str.length(); i++) {
        if (!isalpha(str[i])) {
            w++;
